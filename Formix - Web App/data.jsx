/* global React */
// Mock data + domain helpers for Formix admin
...
const ING = Object.fromEntries(INGREDIENTS.map(i => [i.code, i]));

const RECIPES = [
  {
    code: 'BR-014', name: 'Classic sourdough loaf', description: 'House sourdough, 24h bulk ferment.',
    prepArea: 'bakery', status: 'active', version: 7, updated: '2026-04-18', spec: 'percent',
    lines: [
      { code: 'FLR-001', pct: 60, tol: [3, 3] },
      { code: 'FLR-012', pct: 15, tol: [3, 3] },
      { code: 'WAT-001', pct: 18, tol: [2, 2] },
      { code: 'SLT-001', pct: 1.2, tol: [5, 5] },
      { code: 'YST-002', pct: 0.8, tol: [8, 8] },
      { code: 'OIL-004', pct: 5,    tol: [5, 5] },
    ],
  },
  {
    code: 'BR-021', name: 'Seeded brown bloomer', description: 'Wholemeal with mixed seeds.',
    prepArea: 'bakery', status: 'active', version: 3, updated: '2026-04-11', spec: 'percent',
    lines: [
      { code: 'FLR-012', pct: 58, tol: [3, 3] },
      { code: 'FLR-001', pct: 12, tol: [3, 3] },
      { code: 'WAT-001', pct: 21, tol: [2, 2] },
      { code: 'SLT-001', pct: 1.5, tol: [5, 5] },
      { code: 'YST-002', pct: 1,   tol: [8, 8] },
      { code: 'OIL-004', pct: 6.5, tol: [5, 5] },
    ],
  },
  {
    code: 'SC-008', name: 'Hellenic house mayonnaise', description: 'Mustard-forward mayo, signature line.',
    prepArea: 'sauces', status: 'active', version: 12, updated: '2026-04-22', spec: 'percent',
    lines: [
      { code: 'OIL-004', pct: 74,  tol: [2, 2] },
      { code: 'EGG-001', pct: 14,  tol: [3, 3] },
      { code: 'VIN-001', pct: 6,   tol: [4, 4] },
      { code: 'MST-001', pct: 2.5, tol: [5, 5] },
      { code: 'SLT-001', pct: 1,   tol: [6, 6] },
      { code: 'WAT-001', pct: 2.5, tol: [5, 5] },
    ],
  },
  {
    code: 'SC-011', name: 'Sesame soy dressing', description: 'Asian-style pourable dressing.',
    prepArea: 'sauces', status: 'active', version: 2, updated: '2026-04-03', spec: 'percent',
    lines: [
      { code: 'SOY-001', pct: 42, tol: [3, 3] },
      { code: 'OIL-004', pct: 30, tol: [3, 3] },
      { code: 'SES-001', pct: 8,  tol: [4, 4] },
      { code: 'VIN-001', pct: 12, tol: [4, 4] },
      { code: 'SUG-003', pct: 7,  tol: [5, 5] },
      { code: 'SLT-001', pct: 1,  tol: [6, 6] },
    ],
  },
  {
    code: 'DC-003', name: 'Chocolate ganache filling', description: 'Dark couverture ganache.',
    prepArea: 'dairy', status: 'active', version: 5, updated: '2026-04-09', spec: 'percent',
    lines: [
      { code: 'CCO-001', pct: 48,  tol: [2, 2] },
      { code: 'MLK-002', pct: 34,  tol: [3, 3] },
      { code: 'BUT-001', pct: 14,  tol: [3, 3] },
      { code: 'SUG-003', pct: 4,   tol: [4, 4] },
    ],
  },
  {
    code: 'DC-007', name: 'Hazelnut praline paste', description: 'Roasted hazelnut base.',
    prepArea: 'dairy', status: 'draft', version: 1, updated: '2026-04-23', spec: 'percent',
    lines: [
      { code: 'HZL-001', pct: 58, tol: [2, 2] },
      { code: 'SUG-003', pct: 40, tol: [3, 3] },
      { code: 'SLT-001', pct: 2,  tol: [5, 5] },
    ],
  },
];

const REC = Object.fromEntries(RECIPES.map(r => [r.code, r]));

// Order status: scheduled | in-progress | complete | on-hold | abandoned
const ORDERS = [
...
const QA_CHECKS = [
  ...
  { id: 'q4', name: 'Mixer clean-down signed off',          trigger: 'end-of-mix',     type: 'initials',  area: 'sauces', applies: null, required: true },
  { id: 'q5', name: 'pH reading within spec',               trigger: 'end-of-mix',     type: 'numeric',   area: 'sauces', applies: null, required: true, min: 3.2, max: 4.0 },
  { id: 'q6', name: 'Dough temperature after mix',          trigger: 'end-of-mix',     type: 'numeric',   area: 'bakery', applies: null, required: true, min: 22, max: 26 },
  { id: 'q7', name: 'Visual check — colour & texture',      trigger: 'end-of-mix',     type: 'confirm',   area: 'all',    applies: null, required: false },
];
...
// Audit log (for dashboard recent activity)
const AUDIT = [
...
// ==================== Helpers ====================
function prepAreaName(id) { return (PREP_AREAS.find(p => p.id === id) || {}).name || id; }
function recipe(code)     { return REC[code]; }
function ingredient(code) { return ING[code]; }
function allergenName(id) { return (ALLERGENS.find(a => a.id === id) || {}).name || id; }
function allergenColor(id){ return (ALLERGENS.find(a => a.id === id) || {}).color || '#6b7597'; }

function recipeAllergens(recipeCode) {
  const r = REC[recipeCode]; if (!r) return [];
  const set = new Set();
  r.lines.forEach(l => { (ING[l.code]?.allergens || []).forEach(a => set.add(a)); });
  return [...set];
}

function recipeCost(recipeCode, qty = 100) {
  const r = REC[recipeCode]; if (!r) return 0;
  return r.lines.reduce((acc, l) => {
    const ing = ING[l.code];
    const lineQty = (l.pct / 100) * qty;
    return acc + (ing ? lineQty * ing.cost : 0);
  }, 0);
}

// Pretty format
function fmt(n, dp = 2) { return Number(n).toLocaleString('en-GB', { minimumFractionDigits: dp, maximumFractionDigits: dp }); }
function fmt0(n)        { return Number(n).toLocaleString('en-GB'); }
...
Object.assign(window, {
  PREP_AREAS, ALLERGENS, INGREDIENTS, RECIPES, ORDERS, TERMINALS, USERS, ROLE_META,
  QA_CHECKS, ALLERGEN_RULES, AUDIT, ING, REC,
  prepAreaName, recipe, ingredient, allergenName, allergenColor,
  recipeAllergens, recipeCost, fmt, fmt0, STATUS_CHIP,
});
